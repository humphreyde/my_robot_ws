...
const double c1 = 1.65;  //速度计算参数1
const double c2 = 1.65;  //速度计算参数2
const double xmin = -1.57;   //位置下限
const double xmax = 1.57;    //位置上限


const double vmin = -0.5;   //速度下限
const double vmax = 0.5;    //速度上线
...
struct particle
{
	double x[DATA_SIZE];   //该粒子的当前时刻位置
	double bestx[DATA_SIZE];//该粒子的历史最优位置
	double f;       //该粒子的当前适应度
	double bestf;   //该粒子的历史最优适应度
};   //总共有num个粒子
...
private:
	...
	*/
	double f1(double x[], int dim,Matrix4d T_leftGripper_base_d);
...
public:
	...
	*/
	void PSO(double* best, int DIM,Matrix4d T_leftGripper_base_d);
