...
int main(int argc, char** argv)
{
   ...
   while(!leftarm_state_got)
   {
    ...
    if(is_motion)
    {
        ...
        MatrixXd waypoints_left0(100,7); 
        VectorXd t_0(99); //t_应比waypoints少1
        for(int i = 0; i<99; i++){
        t_0(i) = 1;}

		for(int i = 0; i<100; i++){
        
        waypoints_left0.block<1, 7>(i, 0) = // 0, 0, 0, 0, 0, 0, 0,
                            qInverse100[i].transpose();
						//	
        ...
        cout << "joint_state_test:" << joint_state.transpose()<< endl; 
